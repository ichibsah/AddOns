...
ARMORY_MAX_CONTAINER_ITEMS = 160;
        ...
        for i = 1, numSlots do
            ...
            if ( texture ) then
                if ( Armory:GetInventoryBagLayout() ) then
                    ...
                    if ( id == ARMORY_VOID_CONTAINER ) then
                        -- transpose
                        local voidRows = ARMORY_VOID_STORAGE_MAX / ARMORY_VOID_CONTAINER_COLUMNS;
                        local rowStart = mod(index - 1, voidRows) * ARMORY_VOID_CONTAINER_COLUMNS;
                        local colIndex = mod(floor((index - 1) / voidRows), ARMORY_VOID_CONTAINER_COLUMNS);
                        local pagStart = floor((index - 1) / ARMORY_VOID_STORAGE_MAX) * ARMORY_VOID_STORAGE_MAX;
                        index = rowStart + colIndex + pagStart + 1;
                    end
                else
                    index = offset + i;
                end
