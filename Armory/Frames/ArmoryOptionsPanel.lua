...
function ArmoryOptionsPanel_SetupDependentControl(dependency, control, invert)
    ...
    elseif ( control.type == CONTROLTYPE_DROPDOWN ) then
        ...
        control.Disable = function (self) self.oldDisable(self); (_G[self:GetName().."Text"] or _G[self:GetParent():GetName().."Text"]):SetTextColor(GRAY_FONT_COLOR.r, GRAY_FONT_COLOR.g, GRAY_FONT_COLOR.b) end;
        control.Enable = function (self) self.oldEnable(self); (_G[self:GetName().."Text"] or _G[self:GetParent():GetName().."Text"]):SetTextColor(HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b) end;
    end
end
...
function ArmoryOptionsPanel_EnableDependentControl(control, enable)
    ...
    local swatch = _G[parent:GetName().."ColorSwatch"];
    if ( swatch ) then
        ArmoryOptionsPanel_EnableSwatch(swatch, control:IsEnabled());
    end
end
...
function ArmoryOptionsPanel_ApplyCollapse(control)
    if ( control.value and ArmoryPaperDollOverlayFrame:IsVisible() ) then
...
local function SetArtifacts(on)
    if ( not on ) then
...
function ArmoryOptionsPanel_CheckModule(control, module)
    ...
    elseif ( module == "Artifacts" ) then
        SetArtifacts(control.value);
