            ...
            for realm in pairs(ArmoryDB) do
                for character in pairs(ArmoryDB[realm]) do
                    ...
                    if ( entry.Talents ) then
                        ...
                        local spec = tostring(entry.Specialization1);
                        entry.Talents = {};
                        entry.Talents[spec] = {};
                        self:CopyTable(talents, entry.Talents[spec]);
                    end
                end
            end
            ...
            for realm in pairs(ArmoryDB) do
                for character in pairs(ArmoryDB[realm]) do
                    ...
                    if ( entry.Artifacts ) then
                        local artifacts = {};
                        for titleName, value in pairs(entry.Artifacts) do
                            local textureKit, titleR, titleG, titleB, barConnectedR, barConnectedG, barConnectedB, barDisconnectedR, barDisconnectedG, barDisconnectedB = ArmoryDbEntry.Load(value["ArtInfo"]);
                            value["ArtInfo"] = ArmoryDbEntry.Save(textureKit, titleName, titleR, titleG, titleB, barConnectedR, barConnectedG, barConnectedB, barDisconnectedR, barDisconnectedG, barDisconnectedB);
                            local itemID, altItemID, artifactAppearanceID, appearanceModID, itemAppearanceID, altItemAppearanceID, altOnTop, uiCameraID, altHandUICameraID, modelAlpha, modelDesaturation = ArmoryDbEntry.Load(value["Info"]);
                            value["Info"] = ArmoryDbEntry.Save(altItemID, artifactAppearanceID, appearanceModID, itemAppearanceID, altItemAppearanceID, altOnTop, uiCameraID, altHandUICameraID, modelAlpha, modelDesaturation);
                            artifacts[tostring(itemID)] = value;
                            if ( altItemID ) then
                                artifacts[tostring(altItemID)] = tostring(itemID);
                            end
                        end
...
local detailCounts = {};
function Armory:GetCountDetails(bagCount, bankCount, reagentBankCount, mailCount, auctionCount, altCount, guildCount, equipCount, voidCount, perSlotCount, color)
    local details = "";
    ...
    if ( (reagentBankCount or 0) > 0 ) then
        table.insert(detailCounts, self:FormatCountDetail(REAGENT_BANK, reagentBankCount, color));
    end
    ...
    if ( (guildCount or 0) > 0 ) then
        table.insert(detailCounts, self:FormatCountDetail(GUILD, guildCount, color));
    end
    ...
    if ( (voidCount or 0) > 0 ) then
        table.insert(detailCounts, self:FormatCountDetail(ARMORY_VOID_STORAGE_ABBR, voidCount, color));
    end
            ...
            if ( icon and icon:IsShown() ) then
                ...
                if ( socketType == "meta" ) then
                    ...
                    if ( gemLink ) then
                        ...
                        self:ReleaseTooltip(gemTooltip);
...
function Armory:ArtifactsEnabled(value)
    if ( value ~= nil ) then
        self:SetModule("Artifacts", value);
    end
    return self:GetModule("Artifacts");
end
