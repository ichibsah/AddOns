--[[
    ...
    Revision: e239fe0a1771682204c8cb066520c1348190b5f1 2017-08-05T12:23:58Z
...
local function CellShowTooltipOnly(self, text)
    Armory.summary:SetHighlightTexCoord(0, 0, 0, 0);
    CellShowTooltip(self, text)
end

local function CellHideTooltip(self)
    Armory.summary:SetHighlightTexCoord(0, 1, 0, 1);
    GameTooltip:Hide();
end
...
local function OnMoneyEnter(self, characterInfo)
    ...
    if ( AGB ) then
        local pgbTotal = AGB:GetPGBMoney(characterInfo.RealmInfo.Name, characterInfo.Faction);
        if ( pgbTotal > 0 ) then
			GameTooltip:AddLine(format(ARMORY_MONEY_TOTAL, ARMORY_PERSONAL_GUILDBANK, characterInfo.Faction), "", 1, 1, 1);
...
function Armory:BuildSummary()
    ...
    for _, realm in ipairs(realms) do
        ...
        for _, character in ipairs(self:CharacterList(realm)) do
            ...
            characterInfo.DisplayName = NORMAL_FONT_COLOR_CODE..character..FONT_COLOR_CODE_CLOSE;
    ...
    for _, realmInfo in ipairs(summary) do
        ...
        for _, characterInfo in ipairs(realmInfo.Characters) do
            ...
            if ( not collapsed[realm] ) then
                ...
                myColumn = column; index, column = self.summary:SetCell(index, myColumn, characterInfo.DisplayName);
