--Version 8.2.0   Game Version 8.2.0
local Buttonsetup,lastname, AChange, q, ToggleUnused,Listinfo, temp, Listan
local Cs_Override,MyTokenButton_OnClick,CleanCsorder,AddCsorder,Update_Cs_ordern
local newrow,kaboom
local function eventHandler(self, event, ...)
	if event=="ADDON_LOADED" and ...=="CurrencySorter" then
		...
		end]] --Probably not needed anymore
...
-----Add a few extra for safety
	if TokenFrameContainer.buttons then
		for i=#Listinfo+1,#TokenFrameContainer.buttons+5 do
			Listinfo[i]={}
		end
	end
...
----------------------------------something changed-----------------------------------------
	...
	--Add extra to avoid nil errors
	for i=#Listan+1, numButtons+5 do
		table.insert(Listan,i,i)
	end
	for i=1, numButtons do
		index = offset+i;
		name, isHeader, isExpanded, isUnused, isWatched, count, icon = unpack(Listinfo[Listan[index]] or {});
		button = buttons[i];
		button.check:Hide();
...
-----------------------------------------------------
	...
	hooksecurefunc("TokenFrame_Update",ModTokenFrame_Update)
	hooksecurefunc(TokenFrameContainer,"update",ModTokenFrame_Update)
