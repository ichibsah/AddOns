		...
		for name, missions in pairs(IPMDB.profiles) do
			if name ~= self.profileName and not IPMDB.ignores[name] then
				for i, mission in ipairs(missions) do
					...
					if mission.followerTypeID >= GFT.FollowerType_8_0 or (mission.followerTypeID == GFT.FollowerType_7_0 and IPMDB.enableLegionMissions) or (mission.followerTypeID < GFT.FollowerType_7_0 and IPMDB.enableGarrisonMissions) then
...
local function SetupMissionInfoTooltip(item, isAltMission, anchorFrame)
	...
	if item.followerTypeID == GFT.FollowerType_6_2 then
		...
		-- if (item.followerTypeID == GFT.FollowerType_6_0 and item.level == 100) or (item.followerTypeID == GFT.FollowerType_7_0 and item.level == 110) and item.iLevel > 0 then
	...
	if (item.followers ~= nil) then
		GameTooltip:AddLine(" ");
		GameTooltip:AddLine(item.followerTypeID == GFT.FollowerType_6_2 and _G.GARRISON_SHIPYARD_FOLLOWERS or _G.GARRISON_FOLLOWERS)
		local id, info
		...
		for i = 1, #(item.followers) do
			...
			if type(info) == "table" then
				leftText = nil
				rightText = nil
				if (info.followerTypeID == GFT.FollowerType_7_0 or info.followerTypeID == GFT.FollowerType_8_0) and info.abilities then
...
function addon:HookBFAMissionFrame()
	if _G.BFAMissionFrameMissionsListScrollFrame and not self.BFAMissionsScrollFrame then
		...
		hooksecurefunc(_G.BFAMissionFrame.MissionTab.MissionList, "Update", function()
			if addon.BFAMissionsScrollFrame:IsVisible() and not addon:OrderHallAddonExists() then
				local info, text
				for i, button in ipairs(addon.BFAMissionsScrollFrame.buttons) do
					info = button.info
					text = buttonText[button] or CreateButtonText(button)
					if info and not info.inProgress then
						text:SetText(info.offerEndTime and RAID_INSTANCE_EXPIRES:format(info.offerTimeRemaining) or nil)
